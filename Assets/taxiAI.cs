...
public class taxiAI : MonoBehaviour
{
    ...
    public void Awake()
    {
        check ??= FindObjectOfType<TaxiGameHandler>();
    }
    ...
    public void Update()
    {
        if (!check.completedDelivery && check.closedDialog && !check.tipped)
        {
            pathCreator ??= PathCreator.FindObjectOfType<PathCreator>();
            check ??= checkShit.FindObjectOfType<TaxiGameHandler>();
            ...
            if (!check.tipped && touching) speed = Mathf.Min(speed + 1.5f, 100f);
        ...
        if (check != null && check.rt != null && pathCreator != null && pathCreator.path != null && !check.tipped)
        {
